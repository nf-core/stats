...
#pipeline_version = 3.0 might be needed at some point
```
...
import requests
from report_helpers import bool_indicator, parse_stats_db_json, parse_remote_pipeline_json, calculate_status, calculate_trust_score


pipeline_health_url = "https://nf-co.re/pipelines.json"

# Retrieve pipeline health information from website
response = requests.get(pipeline_health_url, timeout=30, verify=True)
response.raise_for_status()
health_data = response.json()
    
# Load local JSON dump of stats duckDB
with open('data/stats.json', 'r') as f:
    stats_data = json.load(f)

# Parse both data sources
pipeline_health_records = parse_remote_pipeline_json(health_data)
pipeline_records = parse_stats_db_json(stats_data)

# Convert to DataFrames
pipeline_health_df = pd.DataFrame(pipeline_health_records)
pipelines_data = pd.DataFrame(pipeline_records)

# Filter by pipeline name before merging
pipelines_data = pipelines_data[pipelines_data['name'] == pipeline_name]
pipeline_health_df = pipeline_health_df[pipeline_health_df['name'] == pipeline_name]

# Merge the filtered DataFrames on pipeline name
pipelines_data = pd.merge(
    pipelines_data, 
    pipeline_health_df, 
    on='name', 
    how='outer',
...
# For overlapping columns, prefer the original stats data unless it's null
for col in pipeline_health_df.columns:
    if col != 'name':
        health_col = f'{col}_health'
        if health_col in pipelines_data.columns:
            pipelines_data[col] = pipelines_data[col].fillna(pipelines_data[health_col])
            pipelines_data.drop(columns=[health_col], inplace=True)

# Convert date columns to datetime BEFORE getting pipeline_info
...
# Get pipeline info after merge and date conversion
if len(pipelines_data) > 0:
    pipeline_info = pipelines_data.iloc[0]
...
# Update pipeline_info with computed values
if len(pipelines_data) > 0:
    pipeline_info = pipelines_data.iloc[0]
...
# Get boolean metrics with indicators
is_dsl2 = bool_indicator(pipeline_info.get('is_DSL2'))
has_nf_test = bool_indicator(pipeline_info.get('has_nf_test'))
has_schema = bool_indicator(pipeline_info.get('has_schema'))
...
print(f"""| | |
|:---|:---|
| **Pipeline** | {pipeline_info['full_name']} |
| **Tag** | {pipeline_info['last_release_tag']} ({pipeline_info['release_sha'][:7]}) |
...
secret_scanning = bool_indicator(pipeline_info.get('secret_scanning_enabled'))
push_protection = bool_indicator(pipeline_info.get('secret_scanning_push_protection_enabled'))
dependabot = bool_indicator(pipeline_info.get('dependabot_security_updates_enabled'))
validity_checks = bool_indicator(pipeline_info.get('secret_scanning_validity_checks_enabled'))

print(f"""
| Quality Metric | Status |
|:---------------|:------:|
| **DSL2 Pipeline** | {is_dsl2} |
| **nf-test Implemented** | {has_nf_test} |
| **Parameter Schema** | {has_schema} |
| **Modules** | {int(pipeline_info.get('modules_count', 0))} |
| **Subworkflows** | {int(pipeline_info.get('subworkflows_count', 0))} |
